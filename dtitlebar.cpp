 ...
 */
 ...
 /*
  *  Tweaked by @n1coc4cola for the NeoStore.
  */
...
void DTitlebarPrivate::_q_addDefaultMenuItems()
{
    ...
    //Hahaha, this area is still empty... I removed the default items that won't disapear when I set a custom QMenu ;/ ...

    // add help menu item.

    // add about menu item.

    // add quit menu item. menu->addAction(quitAction);
...
void DTitlebar::setVisible(bool visible)
{
    ...
    if (visible) {
        ...
        //Close everything, my app have a bug, that I can't solve. When you click, the basic way don't work, it leaves an invisible window, so, use this.

        connect(d->closeButton, &DWindowCloseButton::clicked, d->targetWindow(), [=]() {Q_EMIT closing(); d->targetWindow()->close(); });
    } else {
        if (!d->targetWindow()) {
            return;
        }
